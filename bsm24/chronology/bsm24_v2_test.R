...
params <- list(
  thick = 20,         # Thickness of sediment slices (in cm) used in the age-depth model
  d.by = 10,          # Depth interval (in cm) at which ages are estimated
  core.name = "bsm24_v2",  # Name of the core being analyzed
  acc.mean = 10,      # Mean accumulation rate (cm per year)
  acc.shape = 2,      # Shape parameter of the accumulation rate distribution
  mem.strength = 0.6, # Strength of memory in accumulation rate changes
  mem.mean = 0.4      # Mean memory (how much accumulation rate depends on previous depth)
)
...
df <- read.csv(
  file = file.path(getwd(), "bsm24_v2", "bsm24_v2.csv"),
  header = TRUE
) %>% 
  mutate(
...
output_dir <- file.path(getwd(), "bsm24_v2")
setwd(output_dir)
write.csv(df, file = "bsm24_v2_cal.csv", row.names = FALSE)
